...
import com.jishijiyu.takeadvantage.entity.result.IntegerPrizeNoteResult;
import com.jishijiyu.takeadvantage.entity.result.IntegerPrizeNoteResult.WrsList;
import com.jishijiyu.takeadvantage.entity.result.IntegerPrizeShowResult;
...
public class MineShinePrizeDesActivity extends Activity {
	...
	private IntegerPrizeNoteResult ipns;
	// 数据list长度
	int size = 0;
	// 有数据按钮可见
	Boolean type = false;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		mContext = MineShinePrizeDesActivity.this;
	...
	private class OnItemSelectedListenerImpl implements OnItemSelectedListener {
		...
		public void onItemSelected(AdapterView<?> parent, View view,
			...
			awardGrade = ipns.p.wrsList.get(position).awardGrade;
			awardId = ipns.p.wrsList.get(position).awardId;
			awardName = ipns.p.wrsList.get(position).awardName;
			periods = ipns.p.wrsList.get(position).periods;
		}
	...
	private void getPrize() {
		gson = new Gson();
		IntegerPrizeNoteRequest ipnq = new IntegerPrizeNoteRequest();
		ipnq.p.tokenId = GetUserIdUtil.getTokenId(mContext);
		ipnq.p.userId = GetUserIdUtil.getUserId(mContext);
		final String updataDes = gson.toJson(ipnq);
		HttpConnectTool.update(updataDes, mContext, new ConnectListener() {

			@Override
			public void contectSuccess(String result) {
				ipns = gson.fromJson(result, IntegerPrizeNoteResult.class);
				if (ipns.p.isTrue) {
					size = ipns.p.wrsList.size();
					for (int i = 0; i < size; i++) {
						list.add(ipns.p.wrsList.get(i).awardName);
					}
					if (size == 0) {
						list.add("请选择奖品");
				...
				.setOnClickListener(new OnClickListener() {
					...
					public void onClick(View v) {
						menuWindow = new SelectPicPopupWindow(
								MineShinePrizeDesActivity.this, itemsOnClick);
						menuWindow.showAtLocation(
								MineShinePrizeDesActivity.this
	...
	private void updata() {
		getImage();
		gson = new Gson();
		IntegerPrizeShowRequest ipsq = new IntegerPrizeShowRequest();
		...
		if (TextUtils.isEmpty(imgUrl)) {
			...
			ipsq.p.tokenId = GetUserIdUtil.getTokenId(mContext);
			ipsq.p.userId = GetUserIdUtil.getUserId(mContext);
			ipsq.p.awardId = awardId + "";
			ipsq.p.awardContent = awardContent;
			ipsq.p.imgUrl = imgUrl;
			ipsq.p.periods = periods;
			ipsq.p.awardGrade = awardGrade + "";
			ipsq.p.awardName = awardName;
			final String updataDes = gson.toJson(ipsq);
			btn_show_prize_des_mine_ok.setClickable(false);
			HttpConnectTool.update(updataDes, mContext, new ConnectListener() {

				@Override
				public void contectSuccess(String result) {
					IntegerPrizeShowResult ipss = gson.fromJson(result,
							IntegerPrizeShowResult.class);
					...
					if (ipss.p.isTrue) {
						if (ipss.p.isSucce) {
							...
							activity.showPopWindows(
									MineShinePrizeDesActivity.this, view, "15",
									false);
