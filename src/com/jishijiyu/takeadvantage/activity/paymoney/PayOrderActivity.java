 ...
 * @author sunaoyang
 */
public class PayOrderActivity extends HeadBaseActivity {
	...
	public void initReplaceView() {
		...
		View view = View.inflate(PayOrderActivity.this,
				R.layout.pay_order_three, null);
		...
		inviteNum = GetUserIdUtil.inviteUserNum(this);
	...
	public void initView(View view) {
		...
		tv_invite2 = (TextView) view.findViewById(R.id.tv_invite2);
	...
	public void initOnclick() {
		...
		have_invite_radio.setOnClickListener(this);
	...
	public void initData() {
		tv_invite.setText(inviteNum / 4 + "");
		tv_invite2.setText(inviteNum + "");
		tv_gold.setText(goldNum / 50 + "");
		tv_gold2.setText(goldNum + "");
		tv_gold3.setText(remainNum * 50 + "");
	...
	public void onClick(View v) {
		...
		case R.id.btn_add:
			...
			if (!TextUtils.isEmpty(newNeedLock)) {
				...
				remainNum = needNum - inviteExchangeNum - goldExchangeNum;
				if (remainNum < 0) {
					remainNum = 0;
				}
				tv_lock.setText(remainNum + "");
				tv_gold3.setText(50 * remainNum + "");
			}
			break;
		case R.id.btn_del:
			newNeedLock = edt_num.getText().toString().trim();
			...
			break;
		case R.id.have_invite_radio:
			if (inviteCheck == false) {
				inviteCheck = true;
				have_invite_radio.setChecked(true);
				inviteExchangeNum = inviteNum / 4;
				remainNum = needNum - inviteExchangeNum - goldExchangeNum;
				if (remainNum < 0) {
					remainNum = 0;
				}
				tv_lock.setText(remainNum + "");
				tv_gold3.setText(50 * remainNum + "");
			} else if (inviteCheck == true) {
				inviteCheck = false;
				have_invite_radio.setChecked(false);
				inviteExchangeNum = 0;
				remainNum = needNum - inviteExchangeNum - goldExchangeNum;
				if (remainNum < 0) {
					remainNum = 0;
		...
		case R.id.btn_conversion:
			...
			alertDialog = DialogUtils
					.payLockDialog(PayOrderActivity.this, this);
		...
		case R.id.btn_exit:
			...
			break;
		case R.id.btn_yes:
			if (inviteCheck == false && goldCheck == false) {
				// remain RMB支付
				// !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
				num = remainNum;
				pay("换锁", "支付换锁", "0.01", GetUserIdUtil.getUserId(this), num
						+ "", "1");
			} else if (inviteCheck == true && goldCheck == false) {
				if (needNum - inviteExchangeNum > 0) {
					// inviteExchangeNum 邀请全部支付
					num = inviteExchangeNum;
					invitation_conversion();

					// remain RMB支付
					// !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
					num = remainNum;
					pay("换锁", "支付换锁", "0.01", GetUserIdUtil.getUserId(this),
							num + "", "1");
				} else {
					// inviteExchangeNum 邀请支付
					num = needNum;
					invitation_conversion();
				}
			} else if (inviteCheck == false && goldCheck == true) {
			...
			} else if (inviteCheck == true && goldCheck == true) {
				if (needNum - goldExchangeNum - inviteExchangeNum > 0) {
					// inviteExchangeNum 邀请全部支付
					num = inviteExchangeNum;
					invitation_conversion();
					...
					pay("换锁", "支付换锁", "0.01", GetUserIdUtil.getUserId(this),
							num + "", "1");
				} else if (inviteExchangeNum - needNum >= 0) {
					// inviteExchangeNum 邀请支付
					num = needNum;
					invitation_conversion();
				} else {
	...
	private void gold_conversion() {
		...
		HttpConnectTool.update(json, this, new ConnectListener() {
					...
					if (goldlocksresult != null) {
						if (goldlocksresult.p.isTrue) {
							if (goldlocksresult.p.isSucce) {
								ToastUtils.makeText(PayOrderActivity.this,
										"兑换成功！", ToastUtils.LENGTH_SHORT)
										.show();
								goldNum = goldNum - num * 50;
			...
			public void contectFailed(String path, String request) {
				ToastUtils.makeText(PayOrderActivity.this, "兑换失败！",
	...
	public void invitation_conversion() {
		gson = new Gson();
		InvitationLocksRequest invitationLocksRequest = new InvitationLocksRequest();
						...
						if (invitationLocksResult.p.isTrue) {
							if (invitationLocksResult.p.isSucce) {
								ToastUtils.makeText(PayOrderActivity.this,
										"兑换成功！", ToastUtils.LENGTH_SHORT)
										.show();
								inviteNum = inviteNum - num * 4;
								// if (needNum - inviteNum / 4 > 0) {
								// remainNum = needNum - inviteNum / 4;
								// }
								goldLockNum = goldLockNum + num;
								// 邀请人数存入
								LoginUserResult loginUserResult = GetUserIdUtil
										.getLogin(PayOrderActivity.this);
								loginUserResult.p.user.inviteUserNum = inviteNum;
								loginUserResult.p.user.goldLockNum = goldLockNum;
								GetUserIdUtil.saveLoginUserResult(
										PayOrderActivity.this, loginUserResult);
								initData();
	...
	public void getSDKVersion() {
		...
		Toast.makeText(PayOrderActivity.this, version, Toast.LENGTH_SHORT)
				.show();
	}
		...
		public void handleMessage(Message msg) {
			...
			case SDK_PAY_FLAG: {
				...
				if (TextUtils.equals(resultStatus, "9000")) {
					ToastUtils.makeText(PayOrderActivity.this, "支付成功!", 0)
							.show();
					...
					LoginUserResult loginUserResult = GetUserIdUtil
							.getLogin(PayOrderActivity.this);
					loginUserResult.p.user.goldLockNum = goldLockNum;
					GetUserIdUtil.saveLoginUserResult(PayOrderActivity.this,
							loginUserResult);
					initData();
					AppManager.getAppManager().finishActivity(
							PayOrderActivity.this);
					...
					if (TextUtils.equals(resultStatus, "8000")) {
						ToastUtils.makeText(PayOrderActivity.this, "支付结果确认中!",
								0).show();

					} else {
						// 其他值就可以判断为支付失败，包括用户主动取消支付，或者系统返回的错误
						ToastUtils.makeText(PayOrderActivity.this, "支付失败!", 0)
								.show();
