...
import com.jishijiyu.takeadvantage.entity.result.AdvertsingCentreResult.PostJustList;
 ...
 * 
 */
public class AdvertisingCentreAdapter extends MyBaseAdapter {
	List<PostJustList> postJustList;
	Activity context;

	public AdvertisingCentreAdapter(Activity context, List<PostJustList> list) {
		super(context, list);
		this.postJustList = list;
	...
	public View initView(int position, View convertView) {
		...
		if (convertView != null) {
			ImageLoaderUtil.loadImage(postJustList.get(position).posterImgUrl,
					viewHolder.item_img);
			viewHolder.item_name
					.setText(postJustList.get(position).posterTitle);
			viewHolder.surplus_days_text.setText("距离过期：5天");
			viewHolder.browse_count.setText(postJustList.get(position).lookNum
					+ "");
			viewHolder.tasks_count.setText(postJustList.get(position).answerNum
					+ "");
			viewHolder.collect_count
					.setText(postJustList.get(position).collectNum + "");
