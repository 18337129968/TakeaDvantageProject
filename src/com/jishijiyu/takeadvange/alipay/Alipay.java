package com.jishijiyu.takeadvange.alipay;
...
public class Alipay {
		...
		public void handleMessage(Message msg) {
			...
			case SDK_PAY_FLAG: {
				...
				if (TextUtils.equals(resultStatus, "9000")) {
					ToastUtils.makeText(context, "支付成功!", Toast.LENGTH_LONG)
							.show();
				} else {
					// 判断resultStatus 为非“9000”则代表可能支付失败
					// “8000”代表支付结果因为支付渠道原因或者系统原因还在等待支付结果确认，最终交易是否成功以服务端异步通知为准（小概率状态）
					if (TextUtils.equals(resultStatus, "8000!")) {
						ToastUtils.makeText(context, "支付等待!" + resultStatus,
								Toast.LENGTH_LONG).show();
					} else {
						// 其他值就可以判断为支付失败，包括用户主动取消支付，或者系统返回的错误
						ToastUtils.makeText(context, "支付失败!" + resultStatus,
								Toast.LENGTH_LONG).show();
					}
				}
				break;
	...
	/**
	 * call alipay sdk pay. 调用SDK支付 context1 当前的Activity commName 商品名称 commDes
	 * 商品描述 commPrice 商品价格
	 */
	public void pay(final Activity context1, String commName, String commDes,
			String commPrice) {
		this.context = context1;
		// 订单
		String orderInfo = getOrderInfo(commName, commDes, commPrice);
